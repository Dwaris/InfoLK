...
def Datum_in(datumStart, x):
    for i in range(x):
        datumStart = naechstesDatum(datumStart)
    return datumStart

def Tage_von_bis(datumStart,datumZiel):
    i = 0
...
def n_alt(datumStart,datumZiel,multiplikator):
    n = Tage_von_bis(datumStart,datumZiel)
    n = int(n*multiplikator)
    return Datum_in(datumStart,n)
